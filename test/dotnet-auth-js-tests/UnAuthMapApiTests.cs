...
public partial class UnAuthMapIdentityApiTests : LoggedTest
{
    ...
    public async Task LoginFailsGivenUnregisteredUser()
    {
        ...
        await AssertProblemAsync(await client.PostAsJsonAsync("/identity/unlogin", new { Email, Password }),
            "Failed");
    }
    ...
    public async Task LoginFailsGivenWrongPassword()
    {
        ...
        await AssertProblemAsync(await client.PostAsJsonAsync("/identity/unlogin", new { Email, Password = "wrong" }),
            "Failed");
    }
    ...
    public async Task CanLoginWithBearerToken(string addIdentityMode)
    {
        ...
        await RegisterAsync(client);
        var loginResponse = await client.PostAsJsonAsync("/identity/unlogin?useCookies=false", new { Email, Password });
    ...
    public async Task CannotLoginWithCookiesWithOnlyCoreServices()
    {
        ...
        await Assert.ThrowsAsync<InvalidOperationException>(()
            => client.PostAsJsonAsync("/identity/unlogin?useCookies=true", new { Email, Password }));
    }
    ...
    public async Task CanAddEndpointsToMultipleRouteGroupsForSameUserType()
    {
        ...
        app.MapGroup("/identity2").MapUnAuthApi<ApplicationUser>();
    ...
    public async Task CanAddEndpointsToMultipleRouteGroupsForMultipleUsersTypes()
    {
        ...
        app.MapGroup("/identity2").MapUnAuthApi<IdentityUser>();
    ...
    public async Task CanEnableAndLoginWithTwoFactor(string addIdentityMode)
    {
        ...
        AssertOk(await client.PostAsJsonAsync("/identity/unlogin", new { Email, Password, twoFactorCode }));
    }
    ...
    public async Task CanLoginWithRecoveryCodeAndDisableTwoFactor()
    {
        ...
        var recoveryLoginResponse = await client.PostAsJsonAsync("/identity/unlogin", new { Email, Password, TwoFactorRecoveryCode = recoveryCodes[0] });
    ...
    public async Task CanResetRecoveryCodes()
    {
        ...
        AssertOk(await client.PostAsJsonAsync("/identity/unlogin", new { Email, Password, TwoFactorRecoveryCode = recoveryCodes[0] }));
        // Cannot reuse codes
        await AssertProblemAsync(await client.PostAsJsonAsync("/identity/unlogin", new { Email, Password, TwoFactorRecoveryCode = recoveryCodes[0] }),
            "Failed");

        var recoveryLoginResponse = await client.PostAsJsonAsync("/identity/unlogin", new { Email, Password, TwoFactorRecoveryCode = recoveryCodes[1] });
        var recoveryLoginContent = await recoveryLoginResponse.Content.ReadFromJsonAsync<JsonElement>();
        var recoveryAccessToken = recoveryLoginContent.GetProperty("access_token").GetString();
        Assert.NotEqual(accessToken, recoveryAccessToken);
        ...
        AssertOk(await client.PostAsJsonAsync("/identity/unlogin", new { Email, Password, TwoFactorRecoveryCode = resetRecoveryCodes[0] }));

        // Even unused codes from before the reset now fail.
        await AssertProblemAsync(await client.PostAsJsonAsync("/identity/unlogin", new { Email, Password, TwoFactorRecoveryCode = recoveryCodes[2] }),
            "Failed");
    }
    ...
    public async Task CanUsePersistentTwoFactorCookies()
    {
        ...
        var sessionLoginResponse = await client.PostAsJsonAsync("/identity/unlogin?useSessionCookies=true", new { Email, Password, twoFactorCode });
        ...
        var sessionLoginResponse2 = await client.PostAsJsonAsync("/identity/unlogin?useCookies=true&useSessionCookies=true", new { Email, Password, twoFactorCode });
        ...
        var persistentLoginResponse = await client.PostAsJsonAsync("/identity/unlogin?useCookies=true", new { Email, Password, twoFactorCode });
        ...
        var headerRememberLoginResponse = await client.PostAsJsonAsync("/identity/unlogin", new { Email, Password, twoFactorCode });
        // this code adds rememnber header
        var loginContent = await headerRememberLoginResponse.Content.ReadFromJsonAsync<JsonElement>();
        var rememberToken = loginContent.GetProperty("remember_token").GetString();
        ...
        var persistentToken2faResponse = await client.PostAsJsonAsync("/identity/manage/2fa", new object());
        var persistentToken2faContent = await persistentToken2faResponse.Content.ReadFromJsonAsync<JsonElement>();
        Assert.True(persistentToken2faContent.GetProperty("isTwoFactorEnabled").GetBoolean());
        Assert.True(persistentToken2faContent.GetProperty("isMachineRemembered").GetBoolean());
    }
    ...
    public async Task CanResetPassword()
    {
        ...
        await AssertProblemAsync(await client.PostAsJsonAsync("/identity/unlogin", new { Email = confirmedEmail, Password }),
            "Failed");

        // But the new password is
        AssertOk(await client.PostAsJsonAsync("/identity/unlogin", new { Email = confirmedEmail, Password = newPassword }));
    }
    ...
    public async Task CanChangeEmail(string addIdentityModes)
    {
        ...
        await AssertProblemAsync(await client.PostAsJsonAsync("/identity/unlogin", new { Email = newEmail, Password }),
            "Failed");
    ...
    public async Task CannotUpdateClaimsDuringInfoPostWithCookies()
    {
        ...
        // We will finally see all the claims updated after logging in again.
        var secondLoginResponse = await client.PostAsJsonAsync("/identity/unlogin?useCookies=true", new { Email = newEmail, Password });
    ...
    public async Task CanChangePasswordWithoutResetEmail()
    {
        ...
        await AssertProblemAsync(await client.PostAsJsonAsync("/identity/unlogin", new { Email, Password }),
            "Failed");
        AssertOk(await client.PostAsJsonAsync("/identity/unlogin", new { Email, Password = newPassword }));
    }
    ...
    public async Task MustSendValidRequestToSendEmailChangeConfirmation()
    {
        ...
        await AssertProblemAsync(await client.PostAsJsonAsync("/identity/unlogin", new { Email = newEmail, Password = newPassword }),
            "Failed");
        ...
        AssertOk(await client.PostAsJsonAsync("/identity/unlogin", new { Email = newEmail, Password = newPassword }));
    }
        ...
        where TContext : DbContext
    {
        ...
        app.MapGroup("/identity").MapUnAuthApi<TUser>();
        ...
        app.MapGet("/cookie",
            (ClaimsPrincipal user) => $"Cookie: Hello, {user.Identity?.Name}!").RequireAuthorization(UnAuthPolicies.CookieOnly);
        app.MapGet("/bearer",
            (ClaimsPrincipal user) => $"Bearer: Hello, {user.Identity?.Name}!").RequireAuthorization(UnAuthPolicies.BearerOnly);
    ...
    private async Task<string> LoginAsync(HttpClient client, string? groupPrefix = null, string? email = null)
    {
        ...
        await client.PostAsJsonAsync($"{groupPrefix}/unlogin", new { email, Password });
        var loginResponse = await client.PostAsJsonAsync("/identity/unlogin", new { email, Password });
        var loginContent = await loginResponse.Content.ReadFromJsonAsync<JsonElement>();
        var accessToken = loginContent.GetProperty("access_token").GetString();
        var refreshToken = loginContent.GetProperty("refresh_token").GetString();
    ...
    private async Task<string> LoginWithEmailConfirmationAsync(HttpClient client, TestEmailSender emailSender, string? groupPrefix = null, string? email = null)
    {
        ...
        await AssertProblemAsync(await client.PostAsJsonAsync($"{groupPrefix}/unlogin", new { email, Password }),
            "NotAllowed");
